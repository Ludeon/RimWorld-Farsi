...
def _reverse_rtl_word_letters(word: str) -> str:
    ...
    """
    placeholders = [
        (m.group(), m.start()) for m in re.finditer(PLACEHOLDER_REGEX, word)
    ]
...
def _create_lam_alef_combo(prev_letter: Optional[str], alef_variant: str) -> str:
    ...
    if (
        prev_letter
        and prev_letter in CONNECTING_LETTERS
        and not re.match(PLACEHOLDER_REGEX, prev_letter)
    ):
...
def _get_contextual_connecting(
    ...
    connects_to_next = (
        next_letter
        and next_letter in (CONNECTING_LETTERS + NON_CONNECTING_LETTERS)
        and not re.match(PLACEHOLDER_REGEX, next_letter)
    )

    if connects_from_prev and connects_to_next:
        return chr(base_code + 3)  # Middle
    if connects_from_prev:
        return chr(base_code + 1)  # Final
    if connects_to_next:
        return chr(base_code + 2)  # Initial
...
def process_xml_file(file_path: str) -> None:
    ...
    with open(file_path, "wb") as f:
        f.write(
            etree.tostring(
                doc, pretty_print=True, encoding="utf-8", xml_declaration=True
            )
        )
